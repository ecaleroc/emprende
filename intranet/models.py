...
class TabTablas(models.Model):
    ...
    def __str__(self):
        return f"{self.nombre} ({self.idtabla})"
    @classmethod
    def get_choices_for_category(cls, id_catalogo_tablas, solo_activos=True):
        queryset = cls.objects.filter(idcatalogotablas=id_catalogo_tablas)
        if solo_activos:
            # Si tus estados activos son 1, cámbialo aquí
            queryset = queryset.filter(estado=19) # O el valor correcto para "activo"
        return list(queryset.order_by('idtabla').values_list('idtabla', 'nombre'))
...
class ArchivoFirma(models.Model):
    ...
    #def __str__(self):
    #    return f"{self.persona} firmó {self.archivo}"
...
class Conversacion(models.Model):
    ...
    estad_lectura = models.BooleanField(db_column='ESTADO_LECTURA', default=False)
...
class EmpresaSocio(models.Model):
    id = models.AutoField(primary_key=True)  # ID interno solo para Django
    idempresa = models.ForeignKey(Empresa, models.DO_NOTHING, db_column='IDEMPRESA')  # Field name made lowercase. The composite primary key (IDEMPRESA, IDPERSONA) found, that is not supported. The first column is selected.
    idpersona = models.ForeignKey('Persona', models.DO_NOTHING, db_column='IDPERSONA')  # Field name made lowercase.
...
class Login(models.Model):
    ...
    class Meta:
        managed = False
        db_table = 'LOGIN'
    def getDatosSesion(IDUSUARIO):
        datin = []
        with connection.cursor() as cursor:
            cursor.execute("SELECT USUARIO.IDUSUARIO, USUARIO.USUARIO, USUARIO.IDPERSONA, USUARIO.ESTADO, USUARIO.BAJA,USUARIO.NROINTENTOS, PERSONA.TIPODOCUMENTO, PERSONA.NRODOCUMENTO, PERSONA.NOMBRES, PERSONA.APELLIDO_PATERNO, PERSONA.APELLIDO_MATERNO, PERSONA.FOTO, PERSONA.FECHA_NACIMIENTO, PERSONA.GENERO, PERSONA.created_at, PERSONA.TIPO_PERSONA, LOGIN.ACCESS_TOKEN from `USUARIO` inner join `PERSONA` on `USUARIO`.`IDPERSONA` = `PERSONA`.`IDPERSONA` inner join `LOGIN` on `LOGIN`.`IDUSUARIO` = `USUARIO`.`IDUSUARIO` where `USUARIO`.`IDUSUARIO` = "+str(IDUSUARIO)+" ORDER BY `LOGIN`.`IDLOGIN` DESC LIMIT 1;")
...
class Opinion(models.Model):
    idopinion = models.AutoField(db_column='IDOPINION', primary_key=True)  # Field name made lowercase.
...
class UnidadOrganicaTrabajaPersonal(models.Model):
    idunidadorganica = models.OneToOneField(UnidadOrganica, models.DO_NOTHING, db_column='IDUNIDADORGANICA', primary_key=True)  # Field name made lowercase. The composite primary key (IDUNIDADORGANICA, IDPERSONAL) found, that is not supported. The first column is selected.
    idpersonal = models.ForeignKey(Personal, models.DO_NOTHING, db_column='IDPERSONAL')  # Field name made lowercase.
    fecha_ingreso = models.DateTimeField(db_column='FECHA_INGRESO', blank=True, null=True)  # Field name made lowercase.
    fecha_finalizacion = models.DateTimeField(db_column='FECHA_FINALIZACION', blank=True, null=True)  # Field name made lowercase.
    estado = models.IntegerField(db_column='ESTADO', blank=True, null=True, db_comment='IDTABLA DE TAB_TABLAS HEREDADO DE CATALOGO_TABLAS')  # Field name made lowercase.
    cargo = models.IntegerField(db_column='CARGO', blank=True, null=True)  # Field name made lowercase.

    class Meta:
        managed = False
        db_table = 'UNIDAD_ORGANICA_TRABAJA_PERSONAL'
        unique_together = (('idunidadorganica', 'idpersonal'),)
